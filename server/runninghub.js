// RunningHub API 代理 — AI App (ComfyUI 工作流) 提交/查询/上传
// 文档: https://www.runninghub.ai/runninghub-api-doc-en/
// 鉴权独立于 comfly: Bearer <RUNNINGHUB_API_KEY>
...
export async function uploadRunninghub({ buffer, filename, auth = {} }) {
  ...
  try {
    ...
    // 响应: { code:200, message:'success', data:{ type, download_url, filename, size } }
    const d = data?.data;
...
function buildNodeInfoList(prompt, params = {}) {
  const nodes = CONFIG.runninghub.nodes;
  const list = [];
  ...
  // 参考图 (LoadImage 节点, fieldValue 用 RunningHub 上传返回的 filename)
...
export async function getRunninghubStatus({ taskId, auth = {} }) {
  ...
  try {
    ...
    if (status === 'SUCCESS') {
      // results: [{ url, nodeId, outputType, text }] — 取第一个视频/图片 URL
