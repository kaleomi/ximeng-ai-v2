// 视频生成代理 — Cloudflare Workers 版 (ark / comfly 两套接口)
...
export async function submitVideo({ prompt, params = {}, flavor, auth = {} }) {
  ...
  if (!apiKey) return { ok: false, error: '未配置 API key,请检查环境变量或在右上角 API 设置中填写' };
  ...
  try {
    ...
    // 服务端轮询兜底 — 只有 ark 走兜底(comfly 由前端轮询)
    if (flavor !== 'ark') {
